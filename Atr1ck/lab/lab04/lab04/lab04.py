...
def merge(lst1, lst2):
    ...
    def help_merge(lst1,lst2,new):
        if len(lst1) == 0:
            ...
            if lst1[0] > lst2[0]:
                return help_merge(lst1,lst2[1:],new + [lst2[0]])
            else:
                return help_merge(lst2, lst1[1:], new + [lst1[0]])
    return help_merge(lst1,lst2,[])
...
def summation(n, term):
    ...
    """
    assert n >= 1
    def help_summation(n, term, sum):
        if n == 1:
            return sum + term(n)
        else:
            return help_summation(n - 1, term, sum + term(n))
    return help_summation(n, term, 0)
...
def count_palindromes(L):
    ...
    2
    """
    return list(map(lambda x: x.lower() == x.lower()[::-1],list(L))).count(True)
